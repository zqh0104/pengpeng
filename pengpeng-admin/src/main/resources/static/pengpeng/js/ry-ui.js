...
(function ($) {
    $.extend({
        ...
        operate: {
			...
			// 修改状态
			editUserStatus: function(id) {
				table.set();
				$.modal.confirm("确定封停该" + table.options.modalName + "账号吗？", function() {
					var url = $.common.isEmpty(id) ? table.options.editUserStatus : table.options.editUserStatus.replace("{id}", id);
					if(table.options.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.pengpeng_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
