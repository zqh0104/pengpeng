package com.pengpeng.framework.config.jwt;
...
import com.pengpeng.framework.util.common.JsonConvertUtil;
...
public class JwtFilter extends BasicHttpAuthenticationFilter implements HandlerInterceptor {
            ...
            try {
                ...
                if (throwable instanceof SignatureVerificationException) {
                    ...
                    if (this.refreshToken(request, response)) {
                        ...
                        //msg = "Token不存在或已过期(" + throwable.getMessage() + ")";
                        msg = "Token不存在或已过期";
                    }
                } else {
                    // 应用异常不为空
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        ...
        String key = "pp:member:activetime:" + memberId;
        redisTemplate.opsForHash().put(key, "lasttime", System.currentTimeMillis());
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        String id = JwtUtil.getClaim(token, Constant.ID);
        ...
        if (redisTemplate.hasKey(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
            ...
            if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                redisTemplate.opsForValue().set(Constant.PREFIX_SHIRO_REFRESH_TOKEN + account, currentTimeMillis,JwtUtil.getRefreshTokenExpireTime(), TimeUnit.SECONDS);
                // 刷新AccessToken，设置时间戳为当前最新时间戳
                token = JwtUtil.sign(account, currentTimeMillis, loginType, id);
                ...
                httpServletResponse.setHeader("Access-Control-Expose-Headers", Constant.AUTHORIZATION);
                return true;
            }
        }
        return false;
    ...
    private void response401(ServletResponse response, String msg) {
        ...
        try (PrintWriter out = httpServletResponse.getWriter()) {
            //String data = JsonConvertUtil.objectToJson(AjaxResult.error("无权访问(Unauthorized):" + msg));
            out.append("{code:201,msg:'登录已过期,请从新登陆.'}");
        } catch (IOException e) {
            log.error("直接返回Response信息出现IOException异常:" + e.getMessage());
            throw new UserTokenException("直接返回Response信息出现IOException异常:" + e.getMessage());
        }
    }
}
